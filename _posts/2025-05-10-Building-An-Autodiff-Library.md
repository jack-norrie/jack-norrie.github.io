...
Which allow us to "back propagate" the adjoint values for children in the compute graph, to their parents. When we pair this with the trivial end vertex evaluation $\bar{f}=1$  and pre-implemented functions for evaluations of the vertex derivatives, i.e.
...
Finally, we need some way to reset our compute graph after we have used our calculated gradients for their desired purpose, e.g. an optimiser step/upate. This can be performed with a simple DFS on teh vertices in the graph.  
...
The next important abstraction is the function abstraction. This has 2 abstract methods, forward and backward. The former is used by the `__call__` method to get the forward compute, and the latter is used to tack on the vertex wise derivative to the vertex of the forward application.
...
Where the second approach has used prefix and postfix products to efficiently compute all the products in $O(n)$.
...
We now define a `Vector` and `Matrix` class. Again we omit the trivial dunder methods, but have shown how the dot product and matrix multiplicaiton operations are defined in terms of our add and multiply function.
...
We now define an abstraction for neural network components. Then we define a `Linear` layer and `Sequential` component which composes layers.
...
As a more difficult test we will now fit a univariate non-linear data generation process using a 10 layer neural network.
